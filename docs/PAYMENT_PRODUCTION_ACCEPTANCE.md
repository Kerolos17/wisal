...
- The owner selected the primary account as the temporary sole payment reviewer and confirmed that it can access payment review in production. This is an owner-attested operating check; local configuration still does not expose the production owner-email setting, so it was not independently automated or recorded with credentials.
