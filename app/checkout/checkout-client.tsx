...
export default function CheckoutClient({ plan, destinations, initialPaymentId = null }: { plan: CheckoutPlan; destinations: PaymentDestination[]; initialPaymentId?: string | null }) {
  ...
  const submit = async () => {
    ...
    if (!policyAccepted) { setMessage(L("اقرأ ملخص المدة والاسترداد والمستند المالي ووافق عليه أولًا", "Read and accept the duration, refund, and payment-document summary first")); setState("error"); return; }
  ...
  return <section className="checkout-shell"><div className="checkout-layout">
    ...
    <main className="checkout-card"><header className="checkout-head"><h2>{L("أرسل التحويل ثم أرفق إثباته", "Transfer, then attach your proof")}</h2><p>{L("اختر وسيلة التحويل التي تناسبك، وانسخ تفاصيل الاستلام بدقة قبل رفع الإيصال.", "Choose your transfer method, copy the receiving details, then upload your proof.")}</p></header>
      ...
      <aside className="checkout-policy" aria-label={L("ملخص شروط الدفع", "Payment terms summary")}><h3>{L("قبل إرسال الطلب", "Before you submit")}</h3><ul><li>{L(`تبدأ مدة ${plan.durationDays} يومًا عند موافقة المراجع، وليس عند التحويل.`, `The ${plan.durationDays}-day term starts on reviewer approval, not on transfer.`)}</li><li>{L("يمكن إلغاء الطلب قبل الموافقة. إذا أرسلت التحويل أو طُلب استرداد بعد الموافقة، افتح تذكرة دعم للمراجعة مع بقاء حقوقك الإلزامية محفوظة.", "You can cancel before approval. If you already transferred funds or request a refund after approval, open a support ticket for review; mandatory consumer rights remain unaffected.")}</li><li>{L("إثبات التحويل المرفوع ليس فاتورة أو إيصالًا ضريبيًا. تواصل مع الدعم قبل التحويل لتأكيد نوع المستند المالي المتاح لحالتك.", "The uploaded transfer proof is not a tax invoice or fiscal receipt. Contact support before transferring to confirm the financial document available for your case.")}</li></ul><div className="checkout-policy-links"><Link href="/terms" target="_blank">{L("قراءة شروط الاستخدام", "Read the terms of use")}<ArrowUpRight aria-hidden="true" /></Link><Link href="/workspace?section=support">{L("فتح تذكرة دعم", "Open a support ticket")}<ArrowUpRight aria-hidden="true" /></Link></div></aside>
      <label className="checkout-policy-consent"><input type="checkbox" checked={policyAccepted} onChange={(event) => setPolicyAccepted(event.target.checked)} /> <span>{L("قرأت ملخص المدة والاسترداد والمستند المالي وأوافق على المتابعة.", "I have read the duration, refund, and payment-document summary and agree to continue.")}</span></label>
      {state === "error" && <p className="checkout-error" role="alert">{message}</p>}<Button variant="primary" tone="ink" className="checkout-submit" disabled={state === "submitting" || !destination} onClick={() => void submit()}>{state === "submitting" ? L("جارٍ الإرسال…", "Submitting…") : L("إرسال طلب المراجعة", "Send for review")}</Button>
    </main>
