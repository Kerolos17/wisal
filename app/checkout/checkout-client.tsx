...
export default function CheckoutClient({ plan, destinations, initialPaymentId = null }: { plan: CheckoutPlan; destinations: PaymentDestination[]; initialPaymentId?: string | null }) {
  ...
  </div>{qrOpen && destination?.qrKey && <div className="checkout-qr-dialog-backdrop" role="presentation" onMouseDown={() => setQrOpen(false)}><section className="checkout-qr-dialog" role="dialog" aria-modal="true" aria-labelledby="checkout-qr-title" onMouseDown={(event) => event.stopPropagation()}><button className="checkout-qr-close" type="button" onClick={() => setQrOpen(false)} aria-label={L("إغلاق رمز QR", "Close QR code")}><X aria-hidden="true" /></button><QrCode aria-hidden="true" /><h2 id="checkout-qr-title">{L("امسح رمز الدفع", "Scan payment QR")}</h2><p>{L("افتح تطبيق الدفع على هاتفك ثم امسح الرمز لإتمام التحويل.", "Open your payment app on your phone, then scan this code to complete the transfer.")}</p><img src={`/api/payment-destinations/${destination.method}/qr`} alt={L(`رمز QR للتحويل عبر ${destination.labelAr}`, `${destination.labelEn} transfer QR code`)} /></section></div>}</section>;
}
