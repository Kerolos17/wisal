...
export default function InvitationClient({ data, previewMode = false }: { data: InvitationData; previewMode?: boolean }) {
  ...
  return (
    <>
      {openingState !== "open" && <section dir={ar ? "rtl" : "ltr"} className={`invite-opening opening-mode-${invitation.openingStyle} opening-theme-${templateArt} opening-concept-${templateConcept} ${openingState === "opening" ? "is-opening" : ""}`} aria-label={t.open}>
        ...
        <div className="opening-card" aria-hidden="true"><small>{guest ? `${t.privateInvite} ${guest.name}` : t.special}</small><strong>{event.brideName} <i>&</i> {event.groomName}</strong><Sparkles /></div>
    ...
    <main ref={contentRef} tabIndex={-1} dir={ar ? "rtl" : "ltr"} lang={locale} inert={openingState !== "open" || undefined} className={`public-invite invite-${invitation.accentColor} guest-template-${templateArt} invite-concept-${templateConcept} layout-${invitation.layoutStyle} ${openingState === "open" ? "invite-revealed" : ""}`}>
      ...
      {(["editorial", "botanical", "cinematic"] as TemplateArt[]).includes(templateArt) ? <section className={`signature-invite-hero signature-${templateArt} concept-${templateConcept} ${coverUrl ? "with-cover" : ""}`} style={coverUrl ? { "--guest-photo": `url(${coverUrl}), ${conceptLqip(templateConcept)}` } as CSSProperties : undefined}>
        <div className="signature-copy">
          ...
          <p className="signature-recipient">{guest ? `${t.privateInvite} ${guest.name}` : ar ? "نحتفل بحبنا" : "We’re getting married"}</p>
