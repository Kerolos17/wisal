...
- **TASK-004 production check (2 September 2026):** `https://wisal-self.vercel.app/robots.txt` still allows `/invite/`; an unauthenticated invitation probe returned `Referrer-Policy: strict-origin-when-cross-origin` and no `X-Robots-Tag`. The deployed site is therefore behind this local fix and must be redeployed before the production acceptance item can be closed.
