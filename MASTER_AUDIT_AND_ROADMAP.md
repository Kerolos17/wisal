...
- **TASK-006 baseline (2 September 2026):** the local non-secret environment check passes and Vercel confirms production is `READY` from commit `658267e`; however, all current production aliases are Vercel-owned and no final custom domain is connected. The environment matrix now records this evidence and retains Vercel Preview/Production configuration, Neon Auth trusted origins, and database separation as explicit release gates.
