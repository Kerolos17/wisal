...
- **TASK-005 technical verification (2 September 2026):** production health is green; payment destination, QR, and payment-status routes return 401 with private no-store caching when unauthenticated. Production has active InstaPay (HTTPS payment link plus stored QR) and Vodafone Cash (configured manual account) destinations. The isolated Neon payment runtime suite now passes 20/20 after applying only the missing additive test-schema changes for `media_blobs` and invitation access mode. The owner designated the primary account as a temporary sole reviewer and attested that it can access payment review in production; this was not independently automated because no authenticated credentials were shared. Public paid launch remains blocked on a second reviewer, account-owner confirmation, legal approval, monitoring, and authenticated browser acceptance.
- **TASK-006 hostname verification (2 September 2026):** the owner approved `https://wisal-self.vercel.app` as the production hostname. Vercel confirms production is `READY` from commit `658267e`; live HTTP checks confirm HTTPS/HSTS and that canonical, Open Graph, JSON-LD, robots, and sitemap use the approved hostname. Vercel Preview/Production configuration, Neon Auth trusted origins, and database separation remain explicit release gates.
- **TASK-007 initial evidence (2 September 2026):** the live sign-in, sign-up, and recovery pages serve the Neon Auth UI as private/no-store responses; a malicious protocol-relative callback destination is redirected safely to the Wisal root. The focused auth/readiness contract suite passes 14/14. Controlled account, mailbox-delivery, session-expiry, and bilingual mobile acceptance are still required before this P1 can close.
- **TASK-009 initial E2E baseline (3 September 2026):** Playwright now protects the public entry point, crawler policy, and private-invitation metadata on Chromium and mobile Chromium. The suite refuses production by default and permits it only with an explicit read-only override; its first production smoke passed 6/6. Authenticated, mutable journeys remain deliberately deferred until an isolated Neon branch and disposable test accounts are available.
- **TASK-008 restore-drill evidence (3 September 2026):** a temporary isolated Lakebase Postgres branch created from production reached ready state and passed read-only schema, aggregate, and relationship checks; core foreign-key checks found zero orphan records. The branch was removed after verification. RPO/RTO approval, a named restore owner, and a quarterly drill remain operational requirements.
- **TASK-007 production configuration evidence (3 September 2026):** Neon Auth is enabled on production and confirms the approved Wisal hostname as a trusted redirect origin. Browser-level read-only auth acceptance now covers rendering sign-in and recovery plus rejecting an external callback destination. Account lifecycle, mailbox delivery, session expiry, and bilingual interaction remain the next controlled-account checks.
- **TASK-010 foundation (3 September 2026):** added a structured logger (`lib/logger.ts`) with key-based redaction for sensitive context (tokens, phone numbers, names, emails, messages, and receipts). Converted core API guards (`lib/api-error.ts`, `lib/public-api-guard.ts`, `app/api/platform-content/route.ts`) to use it and added Next.js startup instrumentation. The existing source-contract suite and production build passed when this foundation was added.
- **TASK-010 remaining release work:** error aggregation, uptime probes, route latency/error metrics, dashboard, tested alert routing, SLO thresholds, and an owned incident runbook are still required. This logging foundation alone does not close OPS-001 or authorize public paid launch.
- **TASK-010 health-monitor baseline (4 September 2026):** a scheduled GitHub Actions probe now verifies the production health contract every 15 minutes and fails for an unavailable application or database; its companion runbook contains the initial response and rollback-safe steps. Named notification ownership, error aggregation, dashboards, alert acknowledgement, and SLOs remain release gates.
- **TASK-010 error-aggregation baseline (4 September 2026):** Sentry now captures unhandled browser and server errors only after a production DSN is configured in Vercel. The integration defaults to no PII, no request data, no breadcrumbs, no replay, and no performance tracing. The owner still must configure the Sentry project, route and acknowledge a synthetic alert, and approve metrics/SLOs before the operations gate closes.
---
